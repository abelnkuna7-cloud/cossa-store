...
const migration = readFileSync(
  "supabase/migrations/20260906190000_add_yoco_live_payment_model.sql",
  "utf8",
);
...
test("live availability is read from the server-side payment control", () => {
  assert.match(checkout, /action === "yoco_live_options"/);
  assert.match(checkout, /yocoLiveState/);
  assert.match(checkout, /state === "active" \|\| state === "commissioning"/);
});
test("commissioning checkout remains restricted to Store administrators", () => {
  assert.match(checkout, /control\.yoco_live_state === "commissioning"/);
  assert.match(checkout, /await requireCossaStoreAdmin\(admin, userData\.user\.id\)/);
});
...
test("live writes are service-role-only and customer reads are owned", () => {
  assert.match(
    migration,
    /grant execute on function public\.create_store_yoco_live_payment_attempt.*to service_role/,
  );
...
test("live checkout never exposes supplier metadata", () => {
  ...
  assert.doesNotMatch(
    liveRequest.slice(0, 500),
    /supplierSku|wholesale|margin|agentId|DMC|Printify|CJ/,
  );
});
