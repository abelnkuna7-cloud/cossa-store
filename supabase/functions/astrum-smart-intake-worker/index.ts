...
function stripHtml(value: string) {
  return compact(decode(value.replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ")));
}
...
function modelToken(name: string) {
  return (name.match(/\b[A-Z]{1,6}\d{2,5}[A-Z0-9-]*\b/i)?.[0] ?? name.split(/\s+/)[0] ?? "").trim();
}
function slugTag(value: string) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
}
function categoryMap(supplierCategory: string | null, title: string) {
  const hay = `${supplierCategory ?? ""} ${title}`.toLowerCase();
  if (/camera|security/.test(hay)) return { primary: "security-smart-home", extras: ["technology-electronics"] };
  if (/watch|fitness band/.test(hay)) return { primary: "technology-electronics", extras: ["sports-fitness", "mobile-accessories"] };
  if (/charger|earbud|headphone|keyboard|power bank|mifi|router|enclosure|bluetooth|usb|network|barcode/.test(hay)) return { primary: "technology-electronics", extras: ["mobile-accessories"] };
  return { primary: "technology-electronics", extras: [] as string[] };
}
...
function extractListItems(html: string) {
  ...
  for (const match of html.matchAll(/<li[^>]*>([\s\S]*?)<\/li>/gi)) {
    ...
    if (text.length >= 8 && text.length <= 180 && !/login|cart|menu|contact|privacy/i.test(text)) out.push(text);
...
function physicalEvidence(text: string) {
  ...
  const weight = text.match(/(?:product\s*)?weight\s*[:\-]?\s*(\d+(?:\.\d+)?)\s*(g|kg)/i);
...
async function fetchHtml(url: string) {
  const response = await fetch(url, { headers: { "User-Agent": "CossaStore-SmartIntake/2.0", Accept: "text/html,application/xhtml+xml" } });
...
async function copyImage(admin: any, intakeId: string, sourceUrl: string) {
  ...
  const { error: uploadError } = await admin.storage.from(IMAGE_BUCKET).upload(path, bytes, { contentType: type, cacheControl: "31536000", upsert: false });
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const refs = Array.isArray(body.refs) ? body.refs.map(String).filter((ref) => SAFE_REFS.has(ref) && !HOLD_REFS.has(ref)) : [...SAFE_REFS];
    const dryRun = body.dryRun === true;
    const maxItems = Math.max(1, Math.min(16, Number(body.maxItems) || refs.length));
    ...
    for (const raw of (rows ?? []).slice(0, maxItems) as Intake[]) {
      ...
      try {
        ...
        if (raw.publication_store_product_id || raw.approval_status === "published") {
          results.push({ ref, status: "already_published", productId: raw.publication_store_product_id });
          continue;
        }
        if (raw.approval_status !== "approved") throw new Error("Only CEO-approved Astrum intakes may be published by this worker.");
        ...
        const title = compact(meta(page.html, "og:title").replace(/\s*[–|-]\s*Experience the difference\s*$/i, "")) || compact(raw.name);
        const short = compact(meta(page.html, "og:description")) || `Official Astrum ${title}.`;
        ...
        const officialImage = meta(page.html, "og:image");
        if (!/^https:\/\/astrum\.co\.za\//i.test(officialImage)) throw new Error("Official Astrum product image could not be verified.");
        const features = extractListItems(page.html).filter((item) => !/^SKU:/i.test(item)).slice(0, 10);
        const evidenceText = stripHtml(page.html);
        const physical = physicalEvidence(evidenceText);
        const categories = categoryMap(raw.supplier_category, title);
        const model = modelToken(title) || modelToken(raw.name);
        const tags = [...new Set([
          "astrum", slugTag(model), slugTag(raw.supplier_category ?? "technology"),
        ...
        if (dryRun) {
          results.push({ ref, status: "dry_run_ready", title, sourceUrl: page.url, officialImage, category: categories.primary, features: features.length, physical });
          continue;
        }

        const hostedImage = await copyImage(admin, raw.id, officialImage);
        const trace = Array.isArray(raw.import_trace) ? raw.import_trace : [];
        const brandTrace = { field: "brand", sourceLabel: "manufacturer official Astrum product page", sourceUrl: page.url, observedAt: new Date().toISOString() };
        const { error: updateError } = await admin.from("store_inventory_intakes").update({
          ...
          brand: "Astrum",
          image_urls: [officialImage],
          source_url: page.url,
          features,
          additional_categories: categories.extras,
          merchandising_tags: tags,
          import_trace: [...trace, brandTrace],
          operational_notes: `Smart Intake 2.0: official Astrum evidence verified for ${ref}; customer image copied into Cossa Store custody before publication.`,
        }).eq("id", raw.id).eq("approval_status", "approved").is("publication_store_product_id", null);
        if (updateError) throw updateError;

        const { data: publishData, error: publishError } = await admin.rpc("publish_store_inventory_intake_with_images", {
          p_intake_id: raw.id,
          p_actor_id: ACTOR_ID,
          p_customer_image_urls: [hostedImage],
        });
        ...
        const evidenceState = physical.kg ? "MANUFACTURER_VERIFIED" : "MISSING";
        ...
        const deliveryRow: Record<string, unknown> = {
          ...
          weight_evidence_state: evidenceState,
          ...
          enrichment_result: { source_url: page.url, supplier_product_ref: ref, evidence_basis: physical.kg || physical.dims ? "manufacturer_verified" : "missing" },
        };
        if (physical.dims) Object.assign(deliveryRow, {
          length_cm: physical.dims.length, width_cm: physical.dims.width, height_cm: physical.dims.height, dimension_kind: "product",
          dimensions_source_url: page.url, dimensions_source_evidence: `Official Astrum page dimensions for ${ref}.`, dimensions_verified_at: new Date().toISOString(),
        });
        if (physical.kg) Object.assign(deliveryRow, {
          weight_kg: physical.kg, weight_source_url: page.url, weight_source_evidence: `Official Astrum page weight for ${ref}.`, weight_verified_at: new Date().toISOString(),
        });
        const { error: deliveryError } = await admin.from("store_product_delivery_attributes").upsert(deliveryRow, { onConflict: "store_product_id" });
        if (deliveryError) throw deliveryError;

        results.push({ ref, status: "published", productId, slug: published?.public_slug, price: Number(raw.selling_price_override), sourceUrl: page.url, hostedImage, physicalReadiness: readiness });
      } catch (error) {
    ...
    return json({ ok: true, dryRun, processed: results.length, results, finishedAt: new Date().toISOString() });
  } catch (error) {
