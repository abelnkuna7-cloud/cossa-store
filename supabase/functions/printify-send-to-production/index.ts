...
function cors(r:Request):HeadersInit{const o=r.headers.get("origin");return{"Access-Control-Allow-Origin":o&&ALLOWED.has(o)?o:"","Access-Control-Allow-Headers":"authorization, x-client-info, apikey, content-type, x-cossa-worker-secret","Access-Control-Allow-Methods":"POST, OPTIONS","Access-Control-Allow-Credentials":"true",Vary:"Origin"}}
...
Deno.serve(async r=>{
 ...
 const url=Deno.env.get("SUPABASE_URL"),pub=Deno.env.get("SUPABASE_PUBLISHABLE_KEY")??Deno.env.get("SUPABASE_ANON_KEY"),svc=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY"),token=Deno.env.get("PRINTIFY_API_TOKEN"),workerSecret=Deno.env.get("STORE_FULFILMENT_WORKER_SECRET");if(!url||!pub||!svc||!token)return json(r,{error:"Production authorization is not configured."},503);
 ...
 try{
  ...
  const internal=Boolean(workerSecret&&r.headers.get("x-cossa-worker-secret")===workerSecret&&bearer===svc);
  if(!internal){const u=await user(r,c);await adminRole(a,u.id)}
  const body=await r.json().catch(()=>({})) as Record<string,unknown>;if(!uuid(body.fulfilmentId))throw new Error("A valid fulfilment ID is required.");
  ...
  const {data:decisions,error:de}=await a.from("store_fulfilment_actions").select("action,actor_user_id,reason,created_at").eq("organisation_id",ORG).eq("store_order_id",o.id).order("created_at",{ascending:false}).limit(1);if(de)throw de;
  const latestDecision=decisions?.[0];if(!latestDecision||latestDecision.action!=="approve_production")throw new Error("Production is blocked until the latest audited fulfilment decision is Approve production.");
  ...
  const productionCost=cents(remote.total_price),shippingCost=cents(remote.total_shipping),taxCost=cents(remote.total_tax)??0;if(productionCost===null||shippingCost===null)throw new Error("Production is blocked because Printify liability could not be calculated safely.");
  const liability=productionCost+shippingCost+taxCost,availableFloat=cents(Deno.env.get("PRINTIFY_AVAILABLE_FULFILMENT_FLOAT_MINOR")),reserveFloor=cents(Deno.env.get("PRINTIFY_FULFILMENT_RESERVE_FLOOR_MINOR"))??0,floatCurrency=(Deno.env.get("PRINTIFY_FULFILMENT_FLOAT_CURRENCY")||"").trim().toUpperCase(),billingCurrency=String(remote.currency??Deno.env.get("PRINTIFY_BILLING_CURRENCY")??"").trim().toUpperCase();
  if(availableFloat===null)throw new Error("Production is blocked until reconciled fulfilment float is available.");if(!floatCurrency||!billingCurrency||floatCurrency!==billingCurrency)throw new Error("Production is blocked because fulfilment-float currency does not match Printify billing currency.");const spendable=Math.max(0,availableFloat-reserveFloor);if(liability>spendable)throw new Error(`Production is blocked: supplier liability ${liability} ${billingCurrency} minor units exceeds spendable fulfilment float ${spendable}.`);
