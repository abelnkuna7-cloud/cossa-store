...
Deno.serve(async r=>{
 ...
 try{
  ...
  const remote=await printifyRequest(`/shops/${PRINTIFY_SHOP_ID}/orders/${encodeURIComponent(String(f.provider_order_id))}.json`,token,{method:"GET"}) as Record<string,unknown>;const remoteStatus=String(remote.status??"");
  ...
  if(["canceled","unfulfillable","has-issues"].includes(remoteStatus))throw new Error(`Production is blocked by Printify order status: ${remoteStatus}.`);
  if(Deno.env.get("PRINTIFY_PRODUCTION_ENABLED")!=="true")return json(r,{authorized:false,dryRun:true,providerOrderId:f.provider_order_id,printifyStatus:remoteStatus,gates:{orderPaid:true,paymentApproved:true,noHold:true,manualApprovalConfirmed:true},killSwitch:"PRINTIFY_PRODUCTION_ENABLED"});
  ...
  const now=new Date().toISOString();const {error:ue}=await a.from("store_fulfilment_orders").update({status:"production_requested",provider_response:{order:remote,send_to_production:response},metadata:{...((f.metadata&&typeof f.metadata==="object"&&!Array.isArray(f.metadata)?f.metadata:{}) as Record<string,unknown>),production_authorized_at:now,production_authorized_by:u.id,manual_approval_attested:true},updated_at:now,last_error:null}).eq("id",f.id);if(ue)throw ue;
  return json(r,{authorized:true,providerOrderId:f.provider_order_id,status:"production_requested",sentToProduction:true});
