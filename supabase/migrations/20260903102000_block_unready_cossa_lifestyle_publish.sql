...
-- custom-artwork Printify production route.
...
begin
  ...
  -- Every available Cossa variant must have its own exact custom-artwork route.
  -- Existing Printify product mappings are intentionally technical fallback only
  -- because they can carry supplier-side artwork that is not Cossa Lifestyle art.
  select count(*)
  ...
  where v.product_id = new.id
    ...
    and not exists (
      ...
      where m.organisation_id = new.organisation_id
        ...
        and nullif(btrim(coalesce(m.blueprint_id, '')), '') is not null
  ...
  if unready_variant_count > 0 then
    raise exception using
      errcode = 'check_violation',
      message = format(
        'COSSA_LIFESTYLE_NOT_PRODUCTION_READY: %s available variant(s) do not have an exact synced Printify custom-artwork mapping.',
