-- A verified live Yoco success is authoritative and terminal. Provider event
-- replays and later negative events remain auditable but cannot downgrade it.
...
begin
  ...
  if a.status='succeeded' then
    update public.store_payment_provider_events set processing_status='ignored',processed_at=now()
      where provider='yoco' and environment='live' and provider_event_id=p_event_id;
    return a;
  end if;
  ...
  if p_event_type='payment.succeeded' and p_payment_status='succeeded' then
    if o.status='paid' then
      update public.store_payment_attempts set status='investigation',failure_code='ORDER_ALREADY_PAID',updated_at=now() where id=a.id returning * into a;
