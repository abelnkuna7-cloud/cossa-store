...
create index if not exists store_funding_auth_active_idx on public.store_fulfilment_funding_authorizations(funding_account_id,state) where state in ('reserved','reconcile_required');
...
begin
 ...
 if found then
   if existing.fulfilment_order_id<>p_fulfilment_order_id or existing.provider<>p_provider or existing.liability_minor<>p_liability_minor or existing.currency<>upper(p_currency) then raise exception 'Idempotency key conflicts with a different funding request'; end if;
 ...
 select coalesce(sum(liability_minor),0) into reserved from public.store_fulfilment_funding_authorizations where funding_account_id=a.id and state in ('reserved','reconcile_required');
