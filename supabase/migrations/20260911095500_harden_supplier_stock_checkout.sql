...
begin
  ...
  -- Legacy supplier products that have not yet been migrated into Smart Intake
  -- retain their existing checkout behaviour. Once a product is intake-managed,
  -- fresh supplier evidence becomes mandatory.
  if not found then
    return new;
  end if;
  ...
  if coalesce(v_intake.supplier_available_stock, 0) <= 0
    or v_status in ('out_of_stock', 'out-of-stock', 'unavailable', 'sold_out', 'sold-out') then
...
revoke all on function private.enforce_supplier_stock_before_store_order_item() from public, anon, authenticated;
grant execute on function private.enforce_supplier_stock_before_store_order_item() to service_role;

drop trigger if exists store_order_items_supplier_stock_guard on public.store_order_items;
create trigger store_order_items_supplier_stock_guard
before insert or update of product_id, quantity
on public.store_order_items
for each row
execute function private.enforce_supplier_stock_before_store_order_item();
