...
import { AliExpressBulkStagePanel } from "@/components/admin/AliExpressBulkStagePanel";
import { Button } from "@/components/ui/button";

export function AliExpressAffiliatePanel({ onStaged }: { onStaged?: () => void }) {
  return (
    <div className="space-y-4">
      <section className="rounded-xl border border-primary/30 bg-card p-4 sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            <div className="mt-4 grid gap-2 text-xs text-muted-foreground sm:grid-cols-2">
              <div className="rounded-lg border border-border bg-secondary/30 p-3">
                ...
                <p className="mt-1">Single-product staging plus controlled bulk tracked-link staging. Every new listing remains Draft until reviewed.</p>
              </div>
              <div className="rounded-lg border border-border bg-secondary/30 p-3">
          ...
          <div className="flex shrink-0 flex-col gap-2 sm:flex-row lg:flex-col">
            <Button asChild>
              <Link to="/admin/catalogue/new">Stage one product</Link>
            </Button>
      ...
      <AliExpressBulkStagePanel onStaged={onStaged} />
    </div>
  );
}
